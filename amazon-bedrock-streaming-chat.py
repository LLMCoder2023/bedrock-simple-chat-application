...
st.set_page_config(page_title="Amazon Bedrock Streaming Chat", layout="wide")
st.title("🤖 Amazon Bedrock Streaming Chat")
...
with st.sidebar:
    ...
    MEMORY_WINDOW = st.slider(
        "Memory Window", min_value=0, max_value=10, value=3, step=1
    )
...
def init_conversationchain() -> ConversationChain:
    ...
    llm = Bedrock(
        model_id="anthropic.claude-v2", model_kwargs=model_kwargs, streaming=True
    )
